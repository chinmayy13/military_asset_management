...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    if (!user.password_hash) {
      ...
      return res
        .status(401)
        .json({ message: "Invalid credentials - Password not set" });
    } else {
...
app.get("/api/bases", async (req, res) => {
  try {
    const result = await pool.query(
      "SELECT * FROM bases WHERE is_active = true",
    );
    res.json(result.rows);
  } catch (error) {
...
app.get("/api/dashboard/metrics", async (req, res) => {
  try {
    ...
    const { purchases_exist, transfers_exist, assignments_exist } =
      tablesExist.rows[0];
    ...
    const equipmentTypesResult = await pool.query(
      "SELECT type_id, type_name FROM equipment_types",
    );
    ...
    const metricsPromises = equipmentTypes.map(async (equipType) => {
      ...
      if (transfers_exist) {
        ...
        `;
        const transfersInResult = await pool.query(transfersInQuery, [
          base_id,
          typeId,
          start_date,
          end_date,
      ...
      if (transfers_exist) {
        ...
        `;
        const transfersOutResult = await pool.query(transfersOutQuery, [
          base_id,
          typeId,
          start_date,
          end_date,
      ...
      if (purchases_exist) {
        ...
        `;
        const previousPurchasesResult = await pool.query(
          previousPurchasesQuery,
          [base_id, typeId, start_date],
        );
      ...
      if (transfers_exist) {
        ...
        `;
        const previousTransfersInResult = await pool.query(
          previousTransfersInQuery,
          [base_id, typeId, start_date],
        );
        openingBalance +=
          parseInt(previousTransfersInResult.rows[0].total) || 0;
        ...
        `;
        const previousTransfersOutResult = await pool.query(
          previousTransfersOutQuery,
          [base_id, typeId, start_date],
        );
        openingBalance -=
          parseInt(previousTransfersOutResult.rows[0].total) || 0;
      }
      ...
      if (assignments_exist) {
        ...
        `;
        const previousAssignmentsResult = await pool.query(
          previousAssignmentsQuery,
          [base_id, typeId, start_date],
        );
        openingBalance -=
          parseInt(previousAssignmentsResult.rows[0].total) || 0;
      }
    ...
    if (type_id) {
      ...
      filteredMockMetrics = generatedMockMetrics.filter(
        (item) => item.type_id === typeIdNum,
      );
      console.log(
        `Filtering by type_id ${typeIdNum}, found ${filteredMockMetrics.length} items`,
      );
    }
...
app.post("/api/purchases", async (req, res) => {
  ...
  try {
    ...
    if (!base_id || !type_id || !quantity || quantity <= 0) {
      return res
        .status(400)
        .json({ message: "Missing required fields or invalid quantity" });
    }
    ...
    const baseCheck = await client.query(
      "SELECT * FROM bases WHERE base_id = $1",
      [base_id],
    );
    ...
    const typeCheck = await client.query(
      "SELECT * FROM equipment_types WHERE type_id = $1",
      [type_id],
    );
    if (typeCheck.rows.length === 0) {
      console.log(
        `Equipment type with ID ${type_id} not found, creating it...`,
      );
      await client.query(
...
app.post("/api/transfers", async (req, res) => {
  try {
    ...
    if (source_base_id === destination_base_id) {
      return res
        .status(400)
        .json({ message: "Source and destination bases cannot be the same." });
    }
    if (quantity <= 0) {
      return res
        .status(400)
        .json({ message: "Transfer quantity must be positive." });
    }
    ...
    const totalTransferredOut =
      transfersOutResult.rows[0]?.total_transferred_out || 0;
    ...
    const totalTransferredIn =
      transfersInResult.rows[0]?.total_transferred_in || 0;

    // Calculate available quantity
    const availableQuantity =
      Number(totalPurchased) +
      Number(totalTransferredIn) -
      Number(totalTransferredOut);
...
app.post("/api/assignments", async (req, res) => {
  try {
    ...
    if (!asset_id || !assigned_to || !base_id || !assignment_date || !status) {
      return res
        .status(400)
        .json({ message: "Missing required assignment fields." });
    }
